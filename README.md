...
JavaScriptからTypeScriptへ機能単位で移行しています。現在は認証・会員・Todo・問い合わせAPIのRequest / Response型、共通HTTPクライアント、業務で使用するPinia Store、起動処理、Router、定数、共通utilityに加え、ログイン画面、共通レイアウト部品、会員一覧・詳細・退会画面をTypeScript化済みです。
