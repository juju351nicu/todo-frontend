...
Swagger / OpenAPIでBackendとのAPI契約を確認しながら、機能単位の構成へ段階的に移行しています。共通HTTP処理は`src/shared/api`、認証機能は`src/features/auth`、会員一覧と会員API・Store・型は`src/features/member`へ移行済みです。
